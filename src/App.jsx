...
export default function App() {
  ...
  const [stickerDataLoaded, setStickerDataLoaded] = useState(false);
  ...
  useEffect(() => {
    ...
    loadStickerData(()=>setStickerDataLoaded(true));
  ...
  return (
    <div style={s.page}>
      ...
      {screen==="mode"    && <Mode me={me} pendingCount={pendingCount} isAdmin={isAdmin}
        ...
        onGo={(sc)=>{ go(sc); }}
        onLogout={signOut}/>}
      {screen==="grid"    && (stickerDataLoaded
        ? <Grid me={me} onBack={()=>go("mode")} onSave={saveStickers} showToast={showToast}/>
        : <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",flexDirection:"column",gap:"1rem",background:"#F0F4FF"}}><div style={{fontSize:"2rem"}}>⚽</div><div style={{color:"#5570A0",fontSize:".9rem"}}>A carregar cromos…</div></div>
      )}
...
function Grid({me, onBack, onSave, showToast}) {
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",paddingBottom:"5rem"}}>
      ...
      {/* Share modal */}
      {showShare && <ShareModal me={meUpdated} onClose={()=>setShowShare(false)} showToast={showToast}/>}
    </div>
  );
}
...
// ════════════════════════════════════════════════════
function ShareModal({me, onClose, showToast}) {
