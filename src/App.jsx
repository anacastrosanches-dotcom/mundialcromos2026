...
// ── PALETA ──────────────────────────────────────────
// Bandeiras — usa twemoji SVG com fallback para badge de texto
const TWEMOJI = "https://cdn.jsdelivr.net/npm/twemoji@14.0.2/assets/svg/";
const FLAG_SVG = {
  USA:"1f1fa-1f1f8",PAN:"1f1f5-1f1e6",CAN:"1f1e8-1f1e6",ECU:"1f1ea-1f1e8",
  ARG:"1f1e6-1f1f7",URU:"1f1fa-1f1fe",MEX:"1f1f2-1f1fd",RSA:"1f1ff-1f1e6",
  CIV:"1f1e8-1f1ee",ENG:"1f3f4-e0067-e0062-e0065-e006e-e0067-e007f",
  AUS:"1f1e6-1f1fa",GER:"1f1e9-1f1ea",SCO:"1f3f4-e0067-e0062-e0073-e0063-e006f-e007f",
  POR:"1f1f5-1f1f9",CRO:"1f1ed-1f1f7",GHA:"1f1ec-1f1ed",MAR:"1f1f2-1f1e6",
  ESP:"1f1ea-1f1f8",BRA:"1f1e7-1f1f7",JPN:"1f1ef-1f1f5",FRA:"1f1eb-1f1f7",
  BEL:"1f1e7-1f1ea",TUN:"1f1f9-1f1f3",PAR:"1f1f5-1f1fe",NED:"1f1f3-1f1f1",
  SEN:"1f1f8-1f1f3",SUI:"1f1e8-1f1ed",KOR:"1f1f0-1f1f7",NOR:"1f1f3-1f1f4",
  KSA:"1f1f8-1f1e6",QAT:"1f1f6-1f1e6",IRQ:"1f1ee-1f1f6",COL:"1f1e8-1f1f4",
  EGY:"1f1ea-1f1ec",JOR:"1f1ef-1f1f4",SWE:"1f1f8-1f1ea",AUT:"1f1e6-1f1f9",
  BIH:"1f1e7-1f1e6",UZB:"1f1fa-1f1ff",HAI:"1f1ed-1f1f9",CPV:"1f1e8-1f1fb",
  CUW:"1f1e8-1f1fc",COD:"1f1e8-1f1e9",IRN:"1f1ee-1f1f7",TUR:"1f1f9-1f1f7",
  CZE:"1f1e8-1f1ff",NZL:"1f1f3-1f1ff",ALG:"1f1e9-1f1ff",PAN:"1f1f5-1f1e6",
  ECU:"1f1ea-1f1e8",
};

const FLAG_COLORS = {
  USA:"#B22234",PAN:"#DA121A",CAN:"#FF0000",ECU:"#FFD100",ARG:"#74ACDF",
  URU:"#75AADB",MEX:"#006847",RSA:"#007A4D",CIV:"#F77F00",ENG:"#CF142B",
  AUS:"#00008B",GER:"#000000",SCO:"#003399",POR:"#006600",CRO:"#FF0000",
  GHA:"#006B3F",MAR:"#C1272D",ESP:"#AA151B",BRA:"#009C3B",JPN:"#BC002D",
  FRA:"#002395",BEL:"#000000",TUN:"#E70013",PAR:"#D52B1E",NED:"#AE1C28",
  SEN:"#00853F",SUI:"#FF0000",KOR:"#003478",NOR:"#EF2B2D",KSA:"#006C35",
  QAT:"#8D1B3D",IRQ:"#CE1126",COL:"#FCD116",EGY:"#CE1126",JOR:"#007A3D",
  SWE:"#006AA7",AUT:"#ED2939",BIH:"#002395",UZB:"#1EB53A",HAI:"#00209F",
  CPV:"#003893",CUW:"#002B7F",COD:"#007FFF",IRN:"#239F40",TUR:"#E30A17",
  CZE:"#D7141A",NZL:"#00247D",ALG:"#006233",
};

function Flag({secId, size=20}) {
  if(secId==="FWC") return <span style={{fontSize:size+"px",lineHeight:1,verticalAlign:"middle"}}>🏆</span>;
  if(secId==="COC") return <span style={{fontSize:size+"px",lineHeight:1,verticalAlign:"middle"}}>🥤</span>;
  const svg = FLAG_SVG[secId];
  const color = FLAG_COLORS[secId]||"#666";
  if(svg) {
    return (
      <img
        src={`${TWEMOJI}${svg}.svg`}
        ...
        height={size}
        style={{display:"inline-block",verticalAlign:"middle",flexShrink:0,borderRadius:2,objectFit:"cover"}}
        onError={e=>{
          // Fallback: coloured badge with country code
          const span = document.createElement('span');
          span.textContent = secId;
          span.style.cssText = `display:inline-block;background:${color};color:white;font-size:${Math.round(size*0.45)}px;font-weight:700;padding:1px 3px;border-radius:2px;vertical-align:middle;letter-spacing:.5px;line-height:${size}px`;
          e.target.replaceWith(span);
        }}
      />
    );
  }
  return <span style={{display:"inline-block",background:color,color:"white",fontSize:Math.round(size*0.45)+"px",fontWeight:700,padding:"1px 3px",borderRadius:2,verticalAlign:"middle",letterSpacing:".5px"}}>{secId}</span>;
}


const C = {
  bg: "#F0F4FF", surf: "#FFFFFF", surf2: "#E8EFFF", surf3: "#D6E3FF",
  green: "#2E9A57", red: "#D94040", gold: "#CC8800", rare: "#9C4A00", legend: "#7B3FA0",
  border: "rgba(60,90,180,0.12)", border2: "rgba(60,90,180,0.20)",
  text: "#1A2A4A", muted: "#5570A0",
};
...
async function loadStickerData(onLoaded) {
  try {
...
function getRarity(secId, num) {
  ...
  if (secId === "FWC" && (num === 0 || num >= 9)) return "special";
...
export default function App() {
  const [screen, setScreen]           = useState("auth"); // auth | mode | grid | search | group | match | market | events | safety | editprofile | sharelist | resetpassword
  ...
  // ── Arranque simples e robusto ──────────────────────────────────
  useEffect(() => {
    // Desinstala qualquer Service Worker anterior que possa estar a causar problemas
    ...
    // Mostra login imediatamente — nunca bloqueia
    setLoading(false);

    // Tenta recuperar sessão guardada localmente (não faz pedido à rede)
    sb.auth.getSession().then(({ data: { session } }) => {
      if (session?.user) {
        setAuthUser(session.user);
        loadMemberProfile(session.user);
      }
    }).catch(() => {}); // ignora erros — login manual resolve

    // Ouve SIGNED_IN, SIGNED_OUT e PASSWORD_RECOVERY
    const { data: { subscription } } = sb.auth.onAuthStateChange((event, session) => {
      if (event === "PASSWORD_RECOVERY") {
        ...
        setAuthUser(null); setMe(null); setIsAdmin(false); go("auth");
      }
    });
  ...
  const saveStickers = async (updated) => {
    try {
      // Verifica sessão actual antes de guardar
      const { data: { session } } = await sb.auth.getSession();
      const userId = session?.user?.id || authUser?.id;
      if(!userId) { showToast("❌ Sessão expirada. Faz login novamente."); go("auth"); return; }

      const { error, count } = await sb.from("members")
        .update({have:updated.have, need:updated.need})
        .eq("auth_id", userId)
        .select();
  ...
  return (
    <div style={s.page}>
      ...
      {screen==="resetpassword" && <ResetPassword showToast={showToast} onDone={()=>{ sb.auth.signOut(); go("auth"); }}/>}
      {toast && <Toast msg={toast}/>}
    </div>
  );
}
// ════════════════════════════════════════════════════
// RESET PASSWORD — Nova password após link de email
// ════════════════════════════════════════════════════
function ResetPassword({showToast, onDone}) {
  ...
  const submit = async () => {
    if(password.length < 6)         { showToast("A password precisa de ter pelo menos 6 caracteres."); return; }
    if(password !== confirm)         { showToast("As passwords não coincidem."); return; }
    setLoading(true);
    const { error } = await sb.auth.updateUser({ password });
    setLoading(false);
    if(error) { showToast("Erro ao actualizar: " + error.message); return; }
    setDone(true);
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"2rem 1.5rem",gap:"1.5rem",background:C.bg}}>
      ...
      <div style={{textAlign:"center"}}>
        <div style={{fontWeight:900,fontSize:"1.8rem"}}>Nova password</div>
        <p style={{color:C.muted,fontSize:".85rem",marginTop:".5rem"}}>Escolhe uma password nova para a tua conta</p>
      </div>
      <div style={{width:"100%",maxWidth:340,background:C.surf,borderRadius:16,padding:"1.25rem",border:`1px solid ${C.border2}`,display:"flex",flexDirection:"column",gap:".85rem"}}>
        {done ? (
          <div style={{textAlign:"center",padding:"1rem 0"}}>
            <div style={{fontSize:"2rem",marginBottom:".5rem"}}>✅</div>
            <div style={{fontWeight:700,marginBottom:".3rem"}}>Password actualizada!</div>
            <div style={{fontSize:".8rem",color:C.muted}}>A redirecionar para o login…</div>
          </div>
        ) : (
          <>
            <div>
              <label style={s.label}>Nova password</label>
              <input style={s.input} type="password" placeholder="mínimo 6 caracteres"
                value={password} onChange={e=>setPassword(e.target.value)}/>
            </div>
            <div>
              <label style={s.label}>Confirmar password</label>
              <input style={s.input} type="password" placeholder="repete a password"
...
function Auth({onSignIn, onSignUp, showToast}) {
  ...
  const sendReset = async () => {
    if(!email.trim()||!email.includes("@")){showToast("Indica o teu email primeiro.");return}
    setLoading(true);
        ...
        <div style={{padding:"1.25rem",display:"flex",flexDirection:"column",gap:".85rem"}}>
          ...
          {tab==="login"&&!resetMode&&(
            <button onClick={()=>setResetMode(true)}
              style={{background:"none",border:"none",color:C.muted,fontSize:".75rem",cursor:"pointer",fontFamily:"inherit",textDecoration:"underline",padding:0,textAlign:"center",width:"100%"}}>
              Esqueci-me da password
            </button>
          )}
          {tab==="login"&&resetMode&&(
            <div style={{background:C.surf2,borderRadius:10,padding:".85rem",border:`1px solid ${C.border2}`}}>
              {resetSent ? (
                <div style={{textAlign:"center"}}>
                  <div style={{fontSize:"1.5rem",marginBottom:".4rem"}}>📧</div>
                  <div style={{fontWeight:700,fontSize:".85rem",marginBottom:".3rem"}}>Email enviado!</div>
                  <div style={{fontSize:".75rem",color:C.muted,lineHeight:1.5}}>Verifica a tua caixa de entrada e segue as instruções para redefinir a password.</div>
                  <button onClick={()=>{setResetMode(false);setResetSent(false);}}
                    style={{marginTop:".75rem",background:"none",border:"none",color:C.muted,fontSize:".75rem",cursor:"pointer",fontFamily:"inherit",textDecoration:"underline",padding:0}}>
                    Voltar ao login
                  </button>
                </div>
              ) : (
                <>
                  <div style={{fontSize:".78rem",color:C.muted,marginBottom:".6rem",lineHeight:1.5}}>
                    Indica o teu email e enviamos um link para recuperares o acesso.
                  </div>
                  <button style={{...s.btn(C.green),width:"100%",marginBottom:".5rem"}} onClick={sendReset} disabled={loading}>
                    {loading?"A enviar…":"Enviar link de recuperação →"}
                  </button>
                  <button onClick={()=>setResetMode(false)}
                    style={{background:"none",border:"none",color:C.muted,fontSize:".75rem",cursor:"pointer",fontFamily:"inherit",textDecoration:"underline",padding:0,width:"100%",textAlign:"center"}}>
                    Cancelar
                  </button>
                </>
              )}
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,padding:"1.5rem"}}>
      <div style={{marginBottom:"1.25rem"}}>
        ...
        {!me?.share_phone && (
          <div style={{marginTop:".6rem",background:"#25D36612",border:"1px solid #25D36630",borderRadius:10,padding:".6rem .85rem",display:"flex",alignItems:"center",gap:".6rem"}}>
            <span style={{fontSize:"1.2rem"}}>📱</span>
            <div style={{flex:1,fontSize:".76rem",color:C.text,lineHeight:1.4}}>
              <strong>Adiciona o teu WhatsApp</strong> para ser mais fácil combinar trocas!
            </div>
            <button onClick={()=>onGo("editprofile")}
              style={{background:"#25D366",color:"white",border:"none",borderRadius:8,padding:".35rem .65rem",fontSize:".73rem",fontWeight:700,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}>
              Adicionar →
            </button>
          </div>
        )}
...
function Grid({me, onBack, onSave, showToast}) {
  ...
  const [stickers, setStickers] = useState(()=>{
    ...
    for(const secId of allSecs){
      s[secId] = {};
      const sec = SECTIONS.find(sec=>sec.id===secId);
      const startNum = secId==="FWC" ? 0 : 1;
      const endNum = sec ? sec.count : 20;
      for(let i=startNum;i<=endNum;i++){
        const hadIt  = (me.have?.[secId]||{})[i]; // antigo: repetido
        const needIt = (me.need?.[secId]||{})[i]; // antigo: em falta
        if(hadIt)        s[secId][i] = hadIt + 1; // repetido → val = qty+1 (ex: 2 cópias → val=3)
        else if(needIt)  s[secId][i] = 0; // em falta → vermelho
        else             s[secId][i] = hasExistingData ? 1 : 0;
      }
    }
    return s;
  });
  ...
  const [secOrder, setSecOrder] = useState("album"); // album | alpha
  const [showTip, setShowTip] = useState(()=>!localStorage.getItem("wc26_tip_seen"));

  // meUpdated — versão actualizada do me com os stickers actuais para o ShareModal
  const meUpdated = useMemo(()=>{
    const have = {}, need = {};
    for(const secId of Object.keys(stickers)){
      have[secId] = {}; need[secId] = {};
      for(const [num, val] of Object.entries(stickers[secId])){
        if(val===2){ have[secId][num]=1; need[secId][num]=0; }
        else if(val===1){ have[secId][num]=0; need[secId][num]=0; }
        else { have[secId][num]=0; need[secId][num]=1; }
      }
    }
    return {...me, have, need};
  }, [stickers, me]);

  // sAfterRep: cromos em "tenho*" (foi repetido, próximo click vai a falta)
  const [sAfterRep, setSAfterRep] = useState({});

  // Ciclo: falta(0)→tenho(1)→repetido(2+)→tenho*(2+)→falta(0)
  const toggle = (secId, num) => {
    const key = secId+"_"+num;
    const cur = (stickers[secId]||{})[num]||0;
    const isAR = !!(sAfterRep[key]);
    if(cur===0){
      setStickers(prev=>({...prev,[secId]:{...(prev[secId]||{}),[num]:1}}));
    } else if(cur===1){
      setStickers(prev=>({...prev,[secId]:{...(prev[secId]||{}),[num]:2}}));
    } else if(cur>=2 && !isAR){
      setSAfterRep(prev=>({...prev,[key]:true}));
    } else {
      setStickers(prev=>({...prev,[secId]:{...(prev[secId]||{}),[num]:0}}));
      setSAfterRep(prev=>({...prev,[key]:false}));
    }
  };

  const addQty = (secId, num) => {
    setStickers(prev=>{
      const cur=(prev[secId]||{})[num]||0;
      if(cur>=2) return {...prev,[secId]:{...(prev[secId]||{}),[num]:cur+1}};
      return prev;
    });
  };
  ...
  const setAllSection = (secId, val) => {
    ...
    setStickers(prev=>{
      const newSec = {};
      for(let i=(sec.id==="FWC"?0:1);i<=sec.count;i++) newSec[i]=val;
  ...
  const handleSave = async () => {
    ...
    for(const secId of Object.keys(stickers)){
      ...
      for(const [num, val] of Object.entries(stickers[secId])){
        const key = secId+"_"+num;
        const isAR = !!(sAfterRep[key]); // tenho* — foi repetido mas agora é tenho
        if(val>=2 && !isAR){ have[secId][num]=val-1; need[secId][num]=0; } // repetido
        else if(val>=1 || isAR){ have[secId][num]=0; need[secId][num]=0; } // tenho
  ...
  // Secções opcionais (ex: COC) não contam para as estatísticas
  const optionalIds = new Set(SECTIONS.filter(s=>s.optional).map(s=>s.id));
  const countHave = Object.entries(stickers).reduce((a,[secId,sec])=>
    optionalIds.has(secId)?a:a+Object.entries(sec).filter(([num,v])=>v>=1).length, 0);
  const countRepeat = Object.entries(stickers).reduce((a,[secId,sec])=>
    optionalIds.has(secId)?a:a+Object.entries(sec).reduce((b,[num,v])=>{
      const isAR = !!(sAfterRep[secId+"_"+num]);
      return b+(v>=2&&!isAR?v-1:0);
    },0), 0);
  const countNeed = Object.entries(stickers).reduce((a,[secId,sec])=>
    optionalIds.has(secId)?a:a+Object.values(sec).filter(v=>v===0).length, 0);

  const visibleSections = useMemo(()=>{
    const q = search.toLowerCase().trim();
    const baseOrder = secOrder==="alpha"
      ? [...SECTIONS].sort((a,b)=>a.id.localeCompare(b.id))
      : SECTIONS; // ordem da caderneta (default)
    const filtered = baseOrder.filter(sec=>{
      if(filter==="RARE") return true;
      if(filter!=="ALL" && filter!==sec.id) return false;
      if(q) return sec.id.toLowerCase()===q ||
    ...
    // Ordena: correspondência exacta de ID primeiro, depois ID parcial, depois nome/jogador
    if(q) {
      filtered.sort((a,b)=>{
        const aExact = a.id.toLowerCase()===q ? 0 : a.id.toLowerCase().startsWith(q) ? 1 : 2;
        const bExact = b.id.toLowerCase()===q ? 0 : b.id.toLowerCase().startsWith(q) ? 1 : 2;
    ...
    <div style={{background:C.bg,minHeight:"100vh",paddingBottom:"5rem"}}>
      ...
      <div style={{display:"flex",gap:0,borderBottom:`1px solid ${C.border}`}}>
        {[
          ...
          ["#2196F3","🔵","Repetidos p/ troca",countRepeat],
      ...
      {/* Tip modal — como funciona o sistema de clicks */}
      {showTip && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",zIndex:150,display:"flex",alignItems:"center",justifyContent:"center",padding:"1.5rem"}}>
          <div style={{background:C.surf,borderRadius:18,padding:"1.5rem",width:"100%",maxWidth:320,border:`1px solid ${C.border2}`}}>
            <div style={{textAlign:"center",fontSize:"1.8rem",marginBottom:".5rem"}}>👆</div>
            <div style={{fontWeight:900,fontSize:"1rem",textAlign:"center",marginBottom:"1rem"}}>Como registar os teus cromos</div>
            <div style={{display:"flex",flexDirection:"column",gap:".6rem",marginBottom:"1.25rem"}}>
              <div style={{display:"flex",alignItems:"center",gap:".75rem",padding:".7rem",background:C.surf2,borderRadius:10,border:`1px solid ${C.border2}`}}>
                <div style={{width:36,height:44,background:"#ff174415",border:`2px solid ${C.red}`,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:"1rem",fontWeight:700,color:C.red}}>?</div>
                <div>
                  <div style={{fontWeight:700,fontSize:".82rem",color:C.red}}>Sem click — Em falta</div>
                  <div style={{fontSize:".75rem",color:C.muted,lineHeight:1.4}}>Não tens este cromo ainda</div>
                </div>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:".75rem",padding:".7rem",background:C.surf2,borderRadius:10,border:`1px solid ${C.border2}`}}>
                <div style={{width:36,height:44,background:"#00c85318",border:`2px solid ${C.green}`,borderRadius:8,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  <div style={{fontSize:".85rem",fontWeight:700,color:C.green}}>1×</div>
                  <div style={{fontSize:".65rem",color:C.green}}>✓</div>
                </div>
                <div>
                  <div style={{fontWeight:700,fontSize:".82rem",color:C.green}}>1 click — Tenho</div>
                  <div style={{fontSize:".75rem",color:C.muted,lineHeight:1.4}}>Tens este cromo no álbum</div>
                </div>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:".75rem",padding:".7rem",background:C.surf2,borderRadius:10,border:`1px solid ${C.border2}`}}>
                <div style={{width:36,height:44,background:"#2196F330",border:`2px solid #2196F3`,borderRadius:8,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  <div style={{fontSize:".85rem",fontWeight:700,color:"#2196F3"}}>2×</div>
                  <div style={{fontSize:".65rem",color:"#2196F3"}}>🔵</div>
                </div>
                <div>
                  <div style={{fontWeight:700,fontSize:".82rem",color:"#2196F3"}}>2 clicks — Repetido</div>
                  <div style={{fontSize:".75rem",color:C.muted,lineHeight:1.4}}>Tens a mais — disponível para troca!</div>
                </div>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:".75rem",padding:".7rem",background:C.surf2,borderRadius:10,border:`1px solid ${C.border2}`}}>
                <div style={{width:36,height:44,background:"#ff174415",border:`2px solid ${C.red}`,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:"1rem",fontWeight:700,color:C.muted}}>✕</div>
                <div>
                  <div style={{fontWeight:700,fontSize:".82rem",color:C.muted}}>3 clicks — Tenho &nbsp;·&nbsp; 4 clicks — Em falta</div>
                  <div style={{fontSize:".75rem",color:C.muted,lineHeight:1.4}}>Clica mais uma vez em Tenho para remover o registo</div>
                </div>
              </div>
            </div>
            <button onClick={()=>{ setShowTip(false); localStorage.setItem("wc26_tip_seen","1"); }}
              style={{...s.btn(C.green),width:"100%",fontWeight:700}}>
              Entendi, vamos lá! →
            </button>
          </div>
        </div>
      ...
      {/* Ordenação + Search */}
      <div style={{padding:".5rem 1rem .2rem",display:"flex",gap:".4rem",alignItems:"center"}}>
        <span style={{fontSize:".72rem",color:C.muted,whiteSpace:"nowrap"}}>Ordem:</span>
        {[["album","📖 Caderneta"],["alpha","🔤 A→Z"]].map(([val,label])=>(
          <button key={val} onClick={()=>setSecOrder(val)}
            style={{background:secOrder===val?C.green:C.surf2,color:secOrder===val?"white":C.text,
              border:`1px solid ${secOrder===val?C.green:C.border2}`,borderRadius:20,
              padding:".25rem .7rem",fontSize:".73rem",fontWeight:secOrder===val?700:500,
              cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}>
            {label}
          </button>
        ))}
      </div>
      <div style={{padding:".3rem 1rem .3rem"}}>
      ...
      <div style={{display:"flex",gap:".4rem",overflowX:"auto",padding:".4rem 1rem .6rem",scrollbarWidth:"none"}}>
        {["ALL","RARE",...(secOrder==="alpha"?[...SECTIONS].sort((a,b)=>a.id.localeCompare(b.id)):SECTIONS).map(s=>s.id)].map(f=>(
          <button key={f} onClick={()=>setFilter(f)} style={{
            ...
            {f==="ALL"?"Todas":f==="RARE"?"⭐ Raros":<span style={{display:"flex",alignItems:"center",gap:4}}><Flag sec={SECTIONS.find(s=>s.id===f)} size={14}/>{f}</span>}
          </button>
        ))}
      </div>
      ...
      {visibleSections.map(sec=>{
        ...
        const nums=[];
        for(let i=(sec.id==="FWC"?0:1);i<=sec.count;i++){
        ...
        return (
          <div key={sec.id} style={{padding:".85rem 1rem .5rem"}}>
            <div style={{display:"flex",alignItems:"center",gap:".6rem",marginBottom:".5rem"}}>
              <Flag sec={sec} size={22}/>
              <div style={{flex:1}}>
                <div style={{fontWeight:700,fontSize:".9rem"}}>{sec.name} <span style={{color:C.muted,fontWeight:400,fontSize:".75rem"}}>{sec.id}</span>{sec.optional&&<span style={{marginLeft:".4rem",fontSize:".68rem",color:C.muted,background:C.surf3,borderRadius:10,padding:".1rem .45rem"}}>opcional</span>}</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(52px,1fr))",gap:".35rem"}}>
              {nums.map(i=>{
                ...
                const isAR=!!(sAfterRep[sec.id+"_"+i]);
                const isLeg=r==="legendary",isSp=r==="special"||r==="coca";
                let bg, bc, numColor;
                if(val>=2&&!isAR){bg="#2196F330";bc="#2196F3";numColor="#2196F3";}
                else if(val===1||isAR){bg="#00c85318";bc=C.green;numColor=C.green;}
                else{
                ...
                return (
                  <div key={i} style={{position:"relative",aspectRatio:"1"}}>
                    <button onClick={()=>toggle(sec.id,i)} style={{
                      width:"100%",height:"100%",background:bg,border:`2px solid ${bc}`,borderRadius:8,
                      ...
                      {(val===1||isAR)&&<span style={{position:"absolute",top:1,right:3,fontSize:".5rem",color:C.green,fontWeight:700}}>✓</span>}
                      {val>=2&&!isAR&&<span style={{position:"absolute",top:1,right:3,fontSize:".5rem",color:"#2196F3",fontWeight:700}}>{val-1}×</span>}
                    </button>
                    {val>=2&&!isAR&&<button onClick={e=>{e.stopPropagation();addQty(sec.id,i);}}
                      style={{position:"absolute",bottom:2,right:2,width:13,height:13,borderRadius:"50%",background:"#2196F3",color:"white",border:"2px solid white",cursor:"pointer",fontSize:9,display:"flex",alignItems:"center",justifyContent:"center",padding:0,lineHeight:1,fontFamily:"inherit",zIndex:2}}>+</button>}
                  </div>
                );
              })}
            </div>
      ...
      {/* Bottom bar — guardar + partilhar */}
      <div style={{position:"fixed",bottom:0,left:0,right:0,padding:".6rem 1rem",background:C.bg,borderTop:`1px solid ${C.border}`,display:"flex",gap:".6rem",alignItems:"center",zIndex:10}}>
        <button onClick={()=>setShowShare(true)}
          style={{background:C.surf2,border:`1px solid ${C.border2}`,borderRadius:10,padding:".55rem .9rem",cursor:"pointer",fontSize:".8rem",fontWeight:600,color:C.text,fontFamily:"inherit",whiteSpace:"nowrap"}}>
          📤 Partilhar
        </button>
        <button style={{...s.btn(saving?"#888":C.green),flex:1,fontWeight:700,fontSize:".9rem",padding:".6rem"}} onClick={handleSave} disabled={saving}>
          {saving?"A guardar…":"Guardar ✓"}
        </button>
      </div>

      {/* Share modal */}
      {showShare && <ShareModal me={meUpdated} secOrder={secOrder} onClose={()=>setShowShare(false)} showToast={showToast}/>}
    </div>
  );
}
    ...
    SECTIONS.forEach(sec=>{
      for(let i=(sec.id==="FWC"?0:1);i<=sec.count;i++){
        ...
        if(name.includes(t)||code.includes(t)||sec.name.toLowerCase().includes(t)||rl.includes(t)){
          const hasRepeat=members.filter(m=>((m.have[sec.id]||{})[i]||0)>=2).map(m=>m.name);
          const needsIt=members.filter(m=>(m.need[sec.id]||{})[i]).map(m=>m.name);
          out.push({sec,i,r,name:PLAYERS[sec.id+i]||sec.id+i,hasRepeat,needsIt});
        }
      }
    });
    ...
    <div style={{background:C.bg,minHeight:"100vh"}}>
      ...
      {results.map(({sec,i,r,name,hasIt,needsIt})=>(
        <div key={sec.id+i} style={{display:"flex",gap:".85rem",padding:".85rem 1rem",borderBottom:`1px solid ${C.border}`}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontWeight:700,fontSize:".9rem",display:"flex",alignItems:"center",gap:".4rem"}}><Flag secId={sec.id} size={16}/>{name}</div>
            ...
            {(hasRepeat.length||needsIt.length)?<div style={{display:"flex",gap:".3rem",flexWrap:"wrap",marginTop:".35rem"}}>
              {hasRepeat.map(n=><span key={n} style={{fontSize:".7rem",background:"#2196F312",color:"#1565C0",borderRadius:20,padding:".15rem .5rem"}}>🔵 {n}</span>)}
...
function Group({me, members, onBack, onOpenMatch}) {
  ...
  const [groupSecOrder, setGroupSecOrder] = useState("album");
  ...
  const filtered = useMemo(() => {
    const list = members.filter(m => {
      if (nameSearch && !m.name.toLowerCase().includes(nameSearch.toLowerCase())) return false;
      ...
      if (matchOnly) {
        ...
        if (Math.min(give.length, get.length) === 0) return false;
      }
      return true;
    });
    if(sortBy!=="alpha" && me){
      list.sort((a,b)=>{
        const ma=computeMatches(me,a), mb=computeMatches(me,b);
        const sa=Math.min(ma.give.length,ma.get.length);
        const sb=Math.min(mb.give.length,mb.get.length);
        return sortBy==="match-desc" ? sb-sa : sa-sb;
      });
    } else {
      list.sort((a,b)=>a.name.localeCompare(b.name,'pt'));
    }
    return list;
  }, [members, cityFilter, mailFilter, matchOnly, nameSearch, sortBy, me]);
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh"}}>
      ...
      <div style={{padding:".6rem 1rem .3rem"}}>
        <input style={{...s.input,fontSize:".85rem"}} placeholder="🔍 Pesquisar pessoa…"
          value={nameSearch} onChange={e=>setNameSearch(e.target.value)}/>
      </div>

      {/* ── FILTROS ── */}
      <div style={{padding:".5rem 1rem .5rem",display:"flex",flexDirection:"column",gap:".6rem",borderBottom:`1px solid ${C.border}`}}>
        <div style={{display:"flex",flexDirection:"column",gap:".5rem"}}>
          <div style={{display:"flex",gap:".4rem",alignItems:"center",flexWrap:"wrap"}}>
            <span style={{fontSize:".72rem",color:C.muted,fontWeight:600}}>Países:</span>
            {[["album","📖 Caderneta"],["alpha","🔤 A→Z"]].map(([val,label])=>(
              <button key={val} onClick={()=>setGroupSecOrder(val)}
                style={{background:groupSecOrder===val?C.green:C.surf2,color:groupSecOrder===val?"white":C.text,
                  border:`1px solid ${groupSecOrder===val?C.green:C.border2}`,borderRadius:20,
                  padding:".22rem .65rem",fontSize:".72rem",fontWeight:groupSecOrder===val?700:500,
          ...
          <div>
            <span style={{fontSize:".72rem",color:C.muted,fontWeight:600,textTransform:"uppercase",letterSpacing:".06em"}}>Ordenar por</span>
          <div style={{display:"flex",gap:".4rem"}}>
            {[["alpha","🔤 A→Z"],["match-desc","🔄 Maior match"],["match-asc","🔄 Menor match"]].map(([val,label])=>(
              <button key={val} onClick={()=>setSortBy(val)}
                style={{background:sortBy===val?C.green:C.surf2,color:sortBy===val?"white":C.text,
                  border:`1px solid ${sortBy===val?C.green:C.border2}`,borderRadius:20,
                  padding:".28rem .75rem",fontSize:".73rem",fontWeight:sortBy===val?700:500,
                  cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}>
                {label}
              </button>
            ))}
      ...
      <div style={{padding:".75rem"}}>
        {filtered.map(m=>{
          ...
          const matchScore=Math.min(matches.give.length,matches.get.length);
          ...
          return (
            <div key={m.name} onClick={()=>!isSelf&&onOpenMatch(m.name)}
              style={{...s.card,marginBottom:".75rem",cursor:isSelf?"default":"pointer",border:matchScore>0?`1px solid ${C.gold}70`:total>0?`1px solid ${C.border2}`:s.card.border}}>
              {/* Header */}
              <div style={{display:"flex",alignItems:"center",gap:".75rem",marginBottom:".5rem"}}>
                ...
                {matchScore>0&&!isSelf&&<span style={{background:C.gold,color:"#000",borderRadius:20,padding:".18rem .65rem",fontSize:".72rem",fontWeight:700}}>🔄 {matchScore}</span>}
              </div>
              {/* Info chips */}
              <div style={{display:"flex",gap:".4rem",flexWrap:"wrap",marginBottom: total>0&&!isSelf ? ".65rem" : 0}}>
              ...
              {/* Matches inline — só quantidades */}
              {total>0&&!isSelf&&(
                <div style={{display:"flex",gap:".5rem",borderTop:`1px solid ${C.border}`,paddingTop:".6rem",flexWrap:"wrap",alignItems:"center"}}>
                  {matches.give.length>0&&(
                    <span style={{fontSize:".78rem",background:"#2196F315",color:"#2196F3",border:"1px solid #2196F330",borderRadius:20,padding:".25rem .75rem",fontWeight:700}}>
                      🔵 Podes dar: {matches.give.length}
                    </span>
                  )}
                  {matches.get.length>0&&(
                    <span style={{fontSize:".78rem",background:"#00c85315",color:C.green,border:`1px solid #00c85330`,borderRadius:20,padding:".25rem .75rem",fontWeight:700}}>
                      🟢 Podes receber: {matches.get.length}
                    </span>
                  )}
                  <span style={{fontSize:".7rem",color:C.muted,marginLeft:"auto"}}>Ver detalhes →</span>
                </div>
              )}
            </div>
          );
...
function MatchDetail({me, members, target, onBack}) {
  ...
  const total = matches.give.length+matches.get.length;
  const matchScore = Math.min(matches.give.length, matches.get.length); // like for like
  const rColor={normal:C.muted,special:C.gold,legendary:C.legend,coca:"#ff6b6b"};
    ...
    <div style={{background:C.bg,minHeight:"100vh"}}>
      ...
      {total===0?<div style={{textAlign:"center",padding:"3rem 1.5rem",color:C.muted}}>
        ...
        <div style={{background:"linear-gradient(135deg,#00c85312,#1565c012)",border:`1px solid #00c85330`,borderRadius:14,margin:"1rem",padding:"1.2rem",textAlign:"center"}}>
          <div style={{fontWeight:900,fontSize:"2.8rem",color:C.gold,lineHeight:1}}>{matchScore}</div>
          <div style={{fontSize:".82rem",color:C.muted,marginTop:".25rem"}}>trocas possíveis · {matches.give.length} que dás · {matches.get.length} que recebes</div>
        </div>
          ...
          {approved.length===0 && (
            <div>
              <div style={{...s.card,marginBottom:".75rem"}}>
                <div style={{fontWeight:700,fontSize:".88rem",marginBottom:".75rem"}}>📍 Pontos de troca em Portugal</div>
                {[
                  {nome:"Colecionar — Amadora",morada:"Av. das Descobertas, Alfragide",nota:"A maior loja de cromos do país. Encontros de trocas regulares aos sábados das 14h às 17h."},
                  {nome:"Colecionar — Lisboa",morada:"Centro Comercial dos Moinhos, Lisboa",nota:"Sessões de trocas organizadas periodicamente."},
                  {nome:"Quiosques do Rossio",morada:"Praça do Rossio, Lisboa",nota:"Vendedores históricos — compra, venda e troca de cromos no local."},
                  {nome:"McDonald's Portugal",morada:"Restaurantes seleccionados",nota:"Ponto oficial de troca Panini aos fins de semana. Ver lista em mcdonalds.pt."},
                  {nome:"Stickerpoint",morada:"stickerpoint.pt (online)",nota:"Venda de cromos avulso — boa opção para completar sem gastar em saquetas."},
                ].map(({nome,morada,nota})=>(
                  <div key={nome} style={{borderTop:`1px solid ${C.border}`,paddingTop:".6rem",marginTop:".6rem"}}>
                    <div style={{fontWeight:600,fontSize:".82rem"}}>{nome}</div>
                    <div style={{fontSize:".74rem",color:C.muted}}>{morada}</div>
                    <div style={{fontSize:".72rem",color:C.muted,marginTop:".2rem",lineHeight:1.5}}>{nota}</div>
                  </div>
                ))}
              </div>
              <div style={{textAlign:"center",padding:"1rem",color:C.muted}}>
                <div style={{fontSize:"2rem",marginBottom:".5rem"}}>📅</div>
                <div style={{fontWeight:700,color:C.text,marginBottom:".4rem"}}>Nenhum encontro do grupo confirmado</div>
                <div style={{fontSize:".85rem"}}>Propõe um encontro na tua zona!</div>
              </div>
            </div>
          )}
          {approved.length>0 && approved.map(ev=><EventCard key={ev.id} ev={ev}/>)}
        </div>
      )}
...
// ════════════════════════════════════════════════════
// SHARE MODAL — Modal de partilha dentro do Grid
// ════════════════════════════════════════════════════
function ShareModal({me, secOrder, onClose, showToast}) {
  const [mode, setMode] = useState("repetidos");

  const generateText = () => {
    const lines = [];
    const orderedSecs = secOrder==="alpha" ? [...SECTIONS].sort((a,b)=>a.id.localeCompare(b.id)) : SECTIONS;
    for(const sec of orderedSecs){
      const nums = [];
      for(let i=(sec.id==="FWC"?0:1);i<=sec.count;i++){
        if(mode==="repetidos"){
          if(((me.have?.[sec.id]||{})[i]||0) >= 2) nums.push(i);
        } else {
          if((me.need?.[sec.id]||{})[i]) nums.push(i);
        }
      }
      if(nums.length>0) lines.push(sec.id + ": " + nums.join(", "));
    }
  ...
  const copy = () => {
    navigator.clipboard.writeText(text)
      .then(()=>{ showToast("Lista copiada! ✓"); onClose(); })
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div style={{background:C.surf,borderRadius:"18px 18px 0 0",width:"100%",maxWidth:480,padding:"1.25rem",paddingBottom:"calc(1.25rem + env(safe-area-inset-bottom))"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1rem"}}>
          <div style={{fontWeight:700,fontSize:"1rem"}}>📤 Partilhar Lista</div>
          <button onClick={onClose} style={{background:"none",border:"none",color:C.muted,fontSize:"1.3rem",cursor:"pointer",padding:0,fontFamily:"inherit"}}>✕</button>
        </div>

        {/* Toggle */}
        <div style={{display:"flex",marginBottom:"1rem",borderRadius:10,overflow:"hidden",border:`1px solid ${C.border2}`}}>
          {[["repetidos","🔵 Repetidos"],["falta","🔴 Em falta"]].map(([id,label])=>(
            <button key={id} onClick={()=>setMode(id)} style={{
              flex:1,padding:".55rem",border:"none",cursor:"pointer",fontFamily:"inherit",
        ...
        {/* Preview */}
        <div style={{background:C.surf2,borderRadius:10,padding:".75rem 1rem",marginBottom:"1rem",maxHeight:200,overflowY:"auto",border:`1px solid ${C.border2}`}}>
          <div style={{fontFamily:"monospace",fontSize:".78rem",lineHeight:1.8,whiteSpace:"pre-wrap",color:C.text}}>
            {text}
          </div>
        </div>

        <div style={{fontSize:".72rem",color:C.muted,marginBottom:".75rem",textAlign:"center"}}>
          {count} {count===1?"equipa":"equipas"}
        </div>

        <div style={{display:"flex",gap:".6rem"}}>
          <button onClick={copy} style={{...s.btn(C.green),flex:2,fontWeight:700}}>
            📋 Copiar
          </button>
          {navigator.share && (
            <button onClick={()=>navigator.share({text})} style={{...s.btn(C.surf2),flex:1,border:`1px solid ${C.border2}`,color:C.text}}>
              📤
            </button>
          )}
        </div>
