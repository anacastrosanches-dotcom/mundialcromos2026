...
// Flag icons CSS — carregado via CDN
const flagIconsStyle = document.createElement('link');
flagIconsStyle.rel = 'stylesheet';
flagIconsStyle.href = 'https://cdn.jsdelivr.net/npm/flag-icons@7.2.3/css/flag-icons.min.css';
document.head.appendChild(flagIconsStyle);
import { createClient } from "https://esm.sh/@supabase/supabase-js@2";

// ── PALETA ──────────────────────────────────────────
// Map section IDs to ISO 3166-1 alpha-2 country codes for flag-icons
const FLAG_CODES = {
  USA:"us",PAN:"pa",CAN:"ca",ECU:"ec",ARG:"ar",PER:"pe",CHI:"cl",ALG:"dz",
  MEX:"mx",RSA:"za",URU:"uy",CIV:"ci",ENG:"gb-eng",AUS:"au",GER:"de",
  SCO:"gb-sct",POR:"pt",CRO:"hr",GHA:"gh",MAR:"ma",ESP:"es",BRA:"br",
  JPN:"jp",CRC:"cr",FRA:"fr",BEL:"be",TUN:"tn",PAR:"py",NED:"nl",SEN:"sn",
  SUI:"ch",KOR:"kr",NOR:"no",KSA:"sa",QAT:"qa",IRQ:"iq",POL:"pl",COL:"co",
  EGY:"eg",JOR:"jo",SWE:"se",AUT:"at",BIH:"ba",UZB:"uz",HAI:"ht",CPV:"cv",
  CUW:"cw",COD:"cd",IRN:"ir",TUR:"tr",CZE:"cz",NZL:"nz",FWC:"",COC:"",
};

function Flag({secId, size=20}) {
  const code = FLAG_CODES[secId];
  if(!code) return <span style={{fontSize:size*0.8+"px"}}>{secId==="FWC"?"🏆":"🥤"}</span>;
  return <span className={`fi fi-${code}`} style={{
    width:size*1.33+"px", height:size+"px",
...
// Componente de bandeira — usa flagcdn.com para imagens reais
const Flag = ({sec, size=20}) => {
  if(!sec?.flagCode) return <span style={{fontSize:size*0.8+"px"}}>{sec?.flag||""}</span>;
  return <img
    src={`https://flagcdn.com/${size*2}x${Math.round(size*1.5)}.png/${sec.flagCode}.png`}
    width={size*1.33} height={size}
    alt={sec.name}
    style={{borderRadius:2,objectFit:"cover",verticalAlign:"middle",flexShrink:0}}
