...
function Grid({me, onBack, onSave, showToast}) {
  ...
  const toggle = (secId, num) => {
    setStickers(prev=>{
      ...
      const next = cur===0 ? 1 : cur===1 ? 2 : 1; // repetido→tenho, não em falta
  ...
  const addQty = (secId, num) => {
    setStickers(prev=>{
      ...
      return {...prev, [secId]:{...(prev[secId]||{}), [num]:cur+1}};
    });
  };

  const resetToFalta = (secId, num) => {
  ...
  const countRepeat = Object.values(stickers).reduce((a,sec)=>
    a+Object.values(sec).reduce((b,v)=>b+(v>=2?v-1:0),0), 0); // total de cópias extra
  const countNeed = Object.values(stickers).reduce((a,sec)=>
    a+Object.values(sec).filter(v=>v===0).length, 0);

  const visibleSections = useMemo(()=>{
    const q = search.toLowerCase().trim();
    const allSorted = [...SECTIONS].sort((a,b)=>a.id.localeCompare(b.id));
    const filtered = allSorted.filter(sec=>{
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",paddingBottom:"5rem"}}>
      ...
      {showTip && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",zIndex:150,display:"flex",alignItems:"center",justifyContent:"center",padding:"1.5rem"}}>
          <div style={{background:C.surf,borderRadius:18,padding:"1.5rem",width:"100%",maxWidth:320,border:`1px solid ${C.border2}`}}>
            ...
            <div style={{fontWeight:900,fontSize:"1rem",textAlign:"center",marginBottom:".3rem"}}>Como registar os teus cromos</div>
            <div style={{fontSize:".75rem",color:C.muted,textAlign:"center",marginBottom:"1rem"}}>Clica em cada cromo para alterar o estado</div>
            <div style={{display:"flex",flexDirection:"column",gap:".5rem",marginBottom:"1.25rem"}}>

              {/* Em falta */}
              <div style={{display:"flex",alignItems:"center",gap:".75rem",padding:".65rem",background:C.surf2,borderRadius:10,border:`1px solid ${C.border2}`}}>
                <div style={{width:36,height:42,background:"#ff174415",border:`2px solid ${C.red}`,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:".95rem",fontWeight:700,color:C.red}}>?</div>
                <div>
                  <div style={{fontWeight:700,fontSize:".82rem",color:C.red}}>Estado inicial — Em falta</div>
                  <div style={{fontSize:".75rem",color:C.muted,lineHeight:1.4}}>Ainda não tens este cromo</div>
                </div>
              </div>

              {/* Tenho */}
              <div style={{display:"flex",alignItems:"center",gap:".75rem",padding:".65rem",background:C.surf2,borderRadius:10,border:`1px solid ${C.border2}`}}>
                <div style={{width:36,height:42,background:"#00c85318",border:`2px solid ${C.green}`,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:".95rem",fontWeight:700,color:C.green}}>✓</div>
                <div>
                  <div style={{fontWeight:700,fontSize:".82rem",color:C.green}}>1 click — Tenho</div>
                  <div style={{fontSize:".75rem",color:C.muted,lineHeight:1.4}}>Tens este cromo colado no álbum</div>
                </div>
              </div>

              {/* Repetido com botão + */}
              <div style={{display:"flex",alignItems:"center",gap:".75rem",padding:".65rem",background:C.surf2,borderRadius:10,border:`1px solid ${C.border2}`}}>
                <div style={{width:36,height:42,background:"#2196F318",border:`2px solid #2196F3`,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,position:"relative"}}>
                  <div style={{fontSize:".82rem",fontWeight:700,color:"#2196F3"}}>+1</div>
                  <div style={{position:"absolute",bottom:-5,right:-5,width:14,height:14,borderRadius:"50%",background:"#2196F3",color:"white",fontSize:10,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700}}>+</div>
                </div>
                <div>
                  <div style={{fontWeight:700,fontSize:".82rem",color:"#2196F3"}}>2 clicks — Repetido</div>
                  <div style={{fontSize:".75rem",color:C.muted,lineHeight:1.4}}>Tens a mais! O botão <strong style={{color:"#2196F3"}}>+</strong> adiciona mais cópias. O contador mostra o total disponível para troca.</div>
                </div>
              </div>

              {/* Botão X */}
              <div style={{display:"flex",alignItems:"center",gap:".75rem",padding:".65rem",background:C.surf2,borderRadius:10,border:`1px solid ${C.border2}`}}>
                <div style={{width:36,height:42,background:"#ff174415",border:`2px solid ${C.red}`,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,position:"relative"}}>
                  <div style={{fontSize:".82rem",fontWeight:700,color:C.green}}>✓</div>
                  <div style={{position:"absolute",top:-5,left:-5,width:14,height:14,borderRadius:"50%",background:C.red,color:"white",fontSize:9,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700}}>✕</div>
                </div>
                <div>
                  <div style={{fontWeight:700,fontSize:".82rem",color:C.muted}}>Botão ✕ — volta a Em falta</div>
                  <div style={{fontSize:".75rem",color:C.muted,lineHeight:1.4}}>Aparece no canto de qualquer cromo marcado</div>
                </div>
              </div>
      ...
      <div style={{display:"flex",gap:".4rem",overflowX:"auto",padding:".4rem 1rem .6rem",scrollbarWidth:"none"}}>
        {["ALL","RARE",...SECTIONS.map(s=>s.id).sort()].map(f=>(
                ...
                return (
                  <div key={i} style={{position:"relative",aspectRatio:"1"}}>
                    <button onClick={()=>toggle(sec.id,i)} style={{
                      ...
                      {val>=2&&<span style={{position:"absolute",top:1,right:3,fontSize:".5rem",color:"#2196F3",fontWeight:700}}>{val-1}×</span>}
                    </button>
                    {val>=1&&<button onClick={e=>{e.stopPropagation();resetToFalta(sec.id,i);}}
                      style={{position:"absolute",top:-5,left:-5,width:14,height:14,borderRadius:"50%",background:C.red,color:"white",border:"none",cursor:"pointer",fontSize:9,display:"flex",alignItems:"center",justifyContent:"center",padding:0,lineHeight:1,fontFamily:"inherit",zIndex:2}}>✕</button>}
                    {val>=2&&<button onClick={e=>{e.stopPropagation();addQty(sec.id,i);}}
                      style={{position:"absolute",bottom:-5,right:-5,width:14,height:14,borderRadius:"50%",background:"#2196F3",color:"white",border:"none",cursor:"pointer",fontSize:10,display:"flex",alignItems:"center",justifyContent:"center",padding:0,lineHeight:1,fontFamily:"inherit",zIndex:2}}>+</button>}
                  </div>
                );
              })}
            </div>
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,padding:".6rem 1rem",background:C.bg,borderTop:`1px solid ${C.border}`,display:"flex",gap:".6rem",alignItems:"center"}}>
  ...
  const filtered = useMemo(() => {
    ...
    if (sortBy==="match" && me) {
      list.sort((a,b) => {
        ...
        return Math.min(mb.give.length,mb.get.length) - Math.min(ma.give.length,ma.get.length);
      });
    } else {
    ...
    <div style={{background:C.bg,minHeight:"100vh"}}>
      ...
      <div style={{padding:".5rem 1rem .5rem",display:"flex",flexDirection:"column",gap:".6rem",borderBottom:`1px solid ${C.border}`}}>
        <div style={{display:"flex",gap:".5rem",alignItems:"center"}}>
          <button onClick={()=>setSortBy(v=>v==="alpha"?"match":"alpha")}
            style={{...s.btnSm(sortBy==="match"?C.gold:C.surf2),fontSize:".75rem",fontWeight:sortBy==="match"?700:500,color:sortBy==="match"?"#000":C.text}}>
            {sortBy==="match"?"🔄 Por match":"🔤 A→Z"}
          </button>
          <span style={{fontSize:".72rem",color:C.muted}}>
            {sortBy==="match"?"Por potencial de troca":"Por ordem alfabética"}
          </span>
        </div>
        {/* Só com matches */}
          ...
          return (
            <div key={m.name} onClick={()=>!isSelf&&onOpenMatch(m.name)}
              ...
              <div style={{display:"flex",alignItems:"center",gap:".75rem",marginBottom:".5rem"}}>
                ...
                {matchScore>0&&!isSelf&&<span style={{background:C.gold,color:"#000",borderRadius:20,padding:".18rem .65rem",fontSize:".72rem",fontWeight:700}}>🔄 {matchScore}+{matchScore}</span>}
                {total>0&&matchScore===0&&!isSelf&&<span style={{background:C.surf3,color:C.muted,borderRadius:20,padding:".18rem .65rem",fontSize:".72rem",fontWeight:700}}>↔ {total}</span>}
              </div>
              {/* Info chips */}
